#!/usr/bin/env python3
import argparse, os, sys, shutil, tempfile, subprocess, textwrap
from pathlib import Path
from concurrent.futures import ThreadPoolExecutor, as_completed

VERSION = "v1.1-py-min-mask"
...
def main():
    ap = argparse.ArgumentParser(
        description="Run GenomeTools LTRharvest over a genome in parallel chunks (with optional low-complexity hardmasking).",
    ...
    ap.add_argument("--annotator", default="LTR_HARVEST_parallel", help="Annotator name in GFF")
    ap.add_argument(
        "--ltrharvest-args",
        default="",
    ...
    # New: masking toggles
    ap.add_argument("--sdust", action="store_true", help="Hard-mask sdust regions per chunk before LTRharvest")
    ap.add_argument("--trf", action="store_true", help="Hard-mask TRF-mod regions per chunk before LTRharvest")
    ap.add_argument("--longdust", action="store_true", help="Hard-mask longdust regions per chunk before LTRharvest")
    ...
    # in main(), add this argument with the others:
    ap.add_argument(
        "--gene-gff",
        default="",
    ...
    def worker(entry):
        ...
        scn_path = chunk_dir / f"{chunk_base}.harvest.scn"
        ...
        if selected_maskers:
            ...
            for name, exe in selected_maskers:
                try:
                    if name == "sdust":
                        # sdust {piece}.fa > {piece}_sdust.bed
                        ...
                        # TRF-mod/trf-mod {piece}.fa -b5 -g5 -s30 -p200 > {piece}_trf.bed
                        ...
                        # longdust/longdust {piece}.fa > {piece}_longdust.bed
        ...
        # ltrharvest
        import shlex
        # Build ltrharvest command: bare by default, or with user-supplied args if provided
    ...
    # Combine .scn and produce GFF features
    lh_args_display = args.ltrharvest_args if args.ltrharvest_args.strip() else "(none)"
    header = textwrap.dedent(f"""\
        #LTR_harvest_parallel -seq {fasta.name} -size {args.size} -time {args.timeout} -threads {args.threads}
    ...
    with open(out_scn, "w") as outscn, open(out_gff, "a") as gff:
        ...
        for (chunk_base, chrom, piece_idx, s0, e0, len_chunk) in list_entries:
            scn_file = chunk_dir / f"{chunk_base}.harvest.scn"
            if not scn_file.exists():
                continue

            coord_adj = s0  # how far into the chromosome this chunk starts
            with open(scn_file) as scn:
                for line in scn:
                    ...
                    if len(parts) < 12:
                        # tolerate shorter tails; try to recover seq_id
                        pass
                    ...
                    gff.write(f"{chrom}\t{args.annotator}\trepeat_region\t{start}\t{end}\t.\t{strand}\t.\tID=repeat_region{count}\n")
                    gff.write(f"{chrom}\t{args.annotator}\tLTR_retrotransposon\t{start}\t{end}\t.\t{strand}\t.\tID=LTR_retrotransposon{count};Parent=repeat_region{count};ltr_identity={sim};seq_number={order_map[chrom]}\n")
                    gff.write(f"{chrom}\t{args.annotator}\tlong_terminal_repeat\t{start}\t{lltr_e}\t.\t{strand}\t.\tParent=LTR_retrotransposon{count}\n")
                    gff.write(f"{chrom}\t{args.annotator}\tlong_terminal_repeat\t{rltr_s}\t{end}\t.\t{strand}\t.\tParent=LTR_retrotransposon{count}\n")
                    gff.write("###\n")
